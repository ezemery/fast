    ...
    <React.Fragment>
      ...
      >
        <div className={`container d-flex ${top ? '' : 'white'}`}>
          ...
          <div
            {...(themeType === '1'
              ? { className: `header-menu ${top ? '' : 'white'}` }
              : {
                  className:
                    type === 'gray' ? 'header2-menu gray' : 'header2-menu',
                })}
          >
            <Menu mode="horizontal">
              ...
              <SubMenu
                title={<span className="submenu-title-wrapper">Company</span>}
              >
                <Menu.ItemGroup>
              ...
              <SubMenu
                title={<span className="submenu-title-wrapper">Platform</span>}
              >
                <Menu.ItemGroup>
                  <Menu.Item key="platform-shopify">
                    <Link href="/platform-shopify">
                      <a>Shopify</a>
                    </Link>
                  </Menu.Item>
                </Menu.ItemGroup>
          ...
          <div
            {...(themeType === '2'
              ? {
                  className: setup
                    ? 'header2-button setup'
                    : `header2-button ${type === 'gray' ? 'gray' : ''}`,
                }
              : { className: `header-button ${top ? '' : 'white'}` })}
          >
            <button className="m-auto btn-fast">{buttonName}</button>
          </div>
          <div
            {...(themeType === '2'
              ? { className: `burger-button ${top ? 'gray' : 'gray'}` }
